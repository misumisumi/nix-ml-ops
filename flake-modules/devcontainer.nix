topLevel@{ flake-parts-lib, inputs, lib, ... }: {
  ...
  flake.flakeModules.devcontainer = flakeModule:
    let
      extraNixDirenvFlakeFlags =
        if builtins.hasAttr "nix-ml-ops" flakeModule.inputs
        then [ "--override-input" "nix-ml-ops/devenv-root" "path:./.devenv/state/pwd" ]
        else if builtins.hasAttr "devenv-root" flakeModule.inputs
        then [ "--override-input" "devenv-root" "path:./.devenv/state/pwd" ]
        else [ ];
    in
    {
      imports = [
                    ...
                    lib.mkMerge
                      ([
                        (lib.mkIf ((builtins.readFile inputs.devenv-root.outPath) == "") ''
                          echo 'Cannot find devenv root file. Please run `nix develop ${lib.escapeShellArgs extraNixDirenvFlakeFlags}` or `direnv reload` instead of `nix develop`.' >&2
                          exit 1
                        '')
